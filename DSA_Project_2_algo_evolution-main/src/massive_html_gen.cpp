...
int main(int argc, char** argv) {
  ...
  html << "        ctx.beginPath();\n";
  html << "        ctx.arc(x, y, 2, 0, Math.PI * 2);\n";
  html << "        ctx.fill();\n";
  html << "      }\n";
  html << "      \n";
  html << "      document.getElementById('genNum').textContent = currentGen + 1;\n";
  html << "      document.getElementById('particleCount').textContent = particlesToShow.length.toLocaleString();\n";
  html << "    }\n";
  html << "    \n";
  html << "    function nextGen() {\n";
  html << "      if (currentGen < maxGenerations - 1) currentGen++;\n";
  html << "      draw();\n";
  html << "    }\n";
  html << "    \n";
  html << "    function prevGen() {\n";
  html << "      if (currentGen > 0) currentGen--;\n";
  html << "      draw();\n";
  html << "    }\n";
  html << "    \n";
  html << "    function togglePlay() {\n";
  html << "      playing = !playing;\n";
  html << "      document.getElementById('playBtn').textContent = playing ? 'Pause' : 'Play';\n";
  html << "    }\n";
  html << "    \n";
  html << "    function reset() {\n";
  html << "      currentGen = 0;\n";
  ...
  html << "    }\n";
  html << "    \n";
  html << "    function showAll() {\n";
  html << "      showAllMode = !showAllMode;\n";
  html << "      document.getElementById('showAllBtn').textContent = showAllMode ? 'Show Top 1K' : 'Show All 100K';\n";
  html << "      draw();\n";
  html << "    }\n";
  html << "    \n";
  html << "    // Click handler for particles\n";
  html << "    canvas.addEventListener('click', function(e) {\n";
  html << "      let rect = canvas.getBoundingClientRect();\n";
  html << "      let x = e.clientX - rect.left;\n";
  html << "      let y = e.clientY - rect.top;\n";
  html << "      \n";
  html << "      if (!data[currentGen]) return;\n";
  html << "      \n";
  html << "      let minDist = Infinity;\n";
  html << "      let closest = null;\n";
  html << "      for (let p of data[currentGen]) {\n";
  html << "        let px = 100 + p[0] * 1200;\n";
  html << "        let py = 100 + p[1] * 600;\n";
  html << "        let dist = Math.sqrt((x - px) * (x - px) + (y - py) * (y - py));\n";
  html << "        if (dist < minDist && dist < 10) {\n";
  html << "          minDist = dist;\n";
  html << "          closest = p;\n";
  html << "        }\n";
  html << "      }\n";
  ...
  html << "      if (closest) {\n";
  html << "        let p = closest;\n";
  html << "        document.getElementById('dnaInfo').innerHTML = \n";
  html << "          'Generation: ' + (currentGen + 1) + '<br>' +\n";
  html << "          'DNA ID: ' + p[7] + '<br>' +\n";
  html << "          'Fitness: ' + p[2].toFixed(4) + ' ms<br>' +\n";
  html << "          'Pivot: ' + (p[4] === 0 ? 'First' : (p[4] === 1 ? 'Last' : 'Median')) + '<br>' +\n";
  html << "          'Partition: ' + (p[5] === 0 ? 'Lomuto' : 'Hoare') + '<br>' +\n";
  html << "          'Cutoff: ' + p[6] + '<br>' +\n";
  html << "          'Comparisons: ' + p[8].toLocaleString() + '<br>' +\n";
  html << "          'Swaps: ' + p[9].toLocaleString();\n";
  html << "        document.getElementById('modal').style.display = 'block';\n";
  html << "      }\n";
  html << "    });\n";
  html << "    \n";
  html << "    function closeModal() {\n";
  html << "      document.getElementById('modal').style.display = 'none';\n";
  html << "    }\n";
  html << "    \n";
  html << "    // Auto-play interval\n";
  html << "    setInterval(function() {\n";
  html << "      if (playing && currentGen < maxGenerations - 1) {\n";
  html << "        nextGen();\n";
  html << "      }\n";
  html << "    }, 2000);\n";
  html << "    \n";
